...
/// Core subsystem name (e.g. 'calendar') to its location, relative to `root`. `None` means the
/// subsystem has no directory of its own (e.g. 'access'), mirroring
/// `\core\component::get_core_subsystems`.
type Subsystems = BTreeMap<String, Option<String>>;
